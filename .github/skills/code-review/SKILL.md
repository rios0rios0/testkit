---
name: code-review
description: "Review pull requests and diffs in testkit — the shared Go builder framework for test data — against the rios0rios0/guide standards, with extra weight on public-API stability, the builder contract, and keeping the module dependency-free. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `testkit`

`testkit` provides the `BaseBuilder`, the `Builder` interface, and `BuilderFactory`/`BuilderConfig` that other repositories use to construct test data. Because it is imported by test suites everywhere, an API change here breaks builds rather than behaviour — and a heavy dependency here is inherited by every consumer.
...
## What matters most in `testkit`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **Every exported symbol in `pkg/test/` is public API.** `BaseBuilder`, `Builder`, `BuilderFactory`, and `BuilderConfig` are compiled against by `autobump`, `autoupdate`, `langforge`, and others. A signature change needs the three-place breaking-change flag and a MAJOR bump.
- **Keep the module dependency-light.** Anything added to `go.mod` here lands in the test build of every consumer; a new dependency needs an explicit justification in the pull-request description.
- **Builders must not leak state between uses.** `Clone` and the reset paths exist so a reused builder cannot carry a value into the next object. A change that shares a map or slice by reference between the original and the clone is a Critical finding — the resulting test flakiness is nearly impossible to trace.
- **Validation errors accumulate, not short-circuit.** `Build` reports every validation failure at once; returning on the first is a behaviour change consumers depend on.
- **`BuilderFactory` is concurrency-safe — keep it that way.** `DefaultFactory` is a package-level singleton reached by every `t.Parallel()` test and the goroutines they spawn, so its registry is guarded by a `sync.RWMutex`. A change that drops the lock, or reads/writes the `builders` map outside it, reintroduces a `fatal error: concurrent map writes` that `recover` cannot catch — a Critical finding. `Create` must keep calling the creation function *after* releasing the lock: the lock is non-reentrant, and a creation function may register further builders.
- **`cmd/example/main.go` is documentation.** A public-API change must be reflected there and in `README.md`, and the example must still run.
- **Coverage is high on purpose** (about 96 %). New exported behaviour without a test is a Warning at minimum — this module's whole job is testing.
- `pkg/test/doc.go` carries package documentation; keep it accurate.
...
- Tests live beside the source in `pkg/test/*_test.go`.
- A new builder capability needs cases for the zero value, the cloned builder, and the validation-failure path.
...
- The example application being trivial — it is a documentation artefact.
